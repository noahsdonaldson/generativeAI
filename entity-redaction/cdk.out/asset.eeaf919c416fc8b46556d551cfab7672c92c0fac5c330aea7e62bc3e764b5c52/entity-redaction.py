...
def redaction(customer_names, entry):
    if not customer_names: 
        return entry
    else:
        words_to_redact = customer_names
        title = entry["title"]
...
def find_entries(entry):
    data = entry["data"]  # need to conform to event data
    
    if data["areaOfContribution"] == "CUSTOMER_ADVISOR":  # need to conform to event data
        ca_entities = detect_entities(data)
        print("entities \n", ca_entities)
        ca_names = return_name(ca_entities)
        temp_names = []
        for ca_name in ca_names:
            temp_names.append(ca_name["name"])
        print("Names gathered \n",temp_names)
        customer_names = is_customer(ca_names)
        print("These are the ones I think are customers \n", customer_names)
        redacted_entry = redaction(customer_names, data)
        print("This is the redacted entry \n", redacted_entry)

    elif data["areaOfContribution"] == "THOUGHT_LEADERSHIP":
        try: 
            data["questions"][5]["Q"] == "Customer feedback or quotes"
            tl_entities = detect_entities(data)
            print("entities \n", tl_entities)
            tl_names = return_name(tl_entities)
            tl_temp_names = []
            for tl_name in tl_names:
                tl_temp_names.append(tl_name["name"])
            print("Names gathered \n", tl_temp_names)
            tl_customer_names = is_customer(tl_names)
            print("These are the ones I think are customers \n", tl_customer_names)
            redacted_entry = redaction(tl_customer_names, data)
            print("This is the redacted entry \n", redacted_entry) 
        except Exception as e:
            print(f"Error {e} on Thought Leadership")
    else:
        print("No customer or thought leadership")
        redacted_entry = data
