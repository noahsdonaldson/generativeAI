...
def find_entries(entry):    
    if entry["areaOfContribution"] == "CUSTOMER_ADVISOR":  # need to conform to event data
        ca_entities = detect_entities(entry)
        print("entities \n", ca_entities)
        ...
        redacted_entry = redaction(customer_names, entry)
        print("This is the redacted entry \n", redacted_entry)

    elif entry["areaOfContribution"] == "THOUGHT_LEADERSHIP":
        try: 
            entry["questions"][5]["Q"] == "Customer feedback or quotes"
            tl_entities = detect_entities(entry)
            print("entities \n", tl_entities)
            ...
            print("These are the ones I think are customers \n", tl_customer_names)
            redacted_entry = redaction(tl_customer_names, entry)
            print("This is the redacted entry \n", redacted_entry) 
